...
**Definition:** Let $(M,g)$ a pseudo-Riemannian Manifold with signature $(p,q)$ and dimension $\dim M = n$. Consider the following set
...
**Proposition:** $(\mathcal{G},\circ)$ is a Lie subgroup of $\text{Aut}(M)$.

**Note:** We can define the angle of two vector fields $X,Y \in \mathfrak{X}(M)$ like so
...
_**Example:**_ In Minkowski space the Poincaré group is a subgroup of $\mathcal{G}$.
...
$$
\begin{align*} \phi : G\times M &\to M\\ (g,p) &\to \phi(g,p) = gp, \end{align*}
$$

with an orbit map $L\_p : G \to M$ for some $p\in M$.

**Definition:** A one parameter subgroup of Lie group $G$ is the maximal integral curve of some left invariant vector field $X \in \mathfrak{g}$ given by the image of the map

$$
\begin{align*} \psi_X: \mathbb{R} &\to G\\ t &\mapsto \exp(tX) \end{align*}
$$
...
![oneparameter](<../../../../../.gitbook/assets/one-parameter-subgroup (1).png>)

**Defintion:** The flow of $p \in M$ according to a one parameter subgroup $\psi\_X$ of $G$ is given by the following curve

$$
\begin{align*} \gamma_p:\mathbb{R} &\to M\\ t &\mapsto e^{tX}p \end{align*}
$$

**Note:** We can rewrite $\gamma\_p$ as $\gamma\_{p,X} = L\_p \circ \psi\_X$.

**Definition:** An infinitesimal transformation of $M$ with respect to the Lie group $G$ in the direction of $X\in \mathfrak{g}$ is given by the map

$$
\begin{align*} F:M &\to M\\ p &\mapsto \gamma_{p,X}(1) \end{align*}
$$

**Corollary:** The map

$$
\begin{align*} L_* : \mathfrak{g} &\to \mathfrak{X}(M)\\ X &\to L_*X \text{ s.t. } (L_*X)_p = L_{p*,e}X \end{align*}
$$
...
**Proposition:** The components of an infinitesimal transformation of a point $p \in \mathbb{R}^n$ are given by

$$
\begin{align*} x^\mu \circ F(p) \coloneqq F(p)^\mu &= p^\mu + X^\nu \frac{\partial L_p^\mu}{\partial \alpha^\nu}, \end{align*}
$$

where $\frac{\partial}{\partial \alpha^\nu}$ is a basis for $\mathfrak{g}$.

**Note:** In physics we usually denote $\varepsilon^\mu = X^\nu \frac{\partial L\_p^\mu}{\partial \alpha^\nu}$.
...
**Definition:** A sequence ${f\_i : G\_{i-1} \to G\_{i\}}\_{i\in I}$ of group homomorphisms is **exact** if the image of each homomorphism is the kernel of the next.

Pictorially we can see this as follows![image-20231004224307511](<../../../../../.gitbook/assets/image-20231004224307511 (1).png>)
...
**Definition:** An **extension** of a some group $G$ by a group $A$ is the group $E$ given by the following exact sequence of group homomorphisms
...
_**Note:**_ The extension of a group by another is not uniquely defined in general.
...
**Definition:** A **Hilbert Space** $\mathbb{H}$ is an inner product space, such that the inner product introduces a norm and a metric, such that $\mathbb{H}$ is complete in the induced topology.

_**Note:**_ Ok a bunch of terms, there. Inner product space, and norm are cool. So is metric. Complete simply means that every convergent sequence of elements in $\mathbb{H}$ converges in $\mathbb{H}$. This is a statement about no missing points. Like don’t try to define one in such a way that I will be following a path and fall out!

**Definition:** A **hermitian inner product** on a vector space $V$ over $\mathbb{C}$ is a map
...
**Corollary:** A complex vector space $V$ together with a **hermitian inner product** has an induced norm given by

$$
\begin{align*} |\cdot|:V &\to \mathbb{R}\\ x & \mapsto \sqrt{\langle x,x\rangle} \end{align*}
$$

**Corollary:** The norm induces the metric topology on $V$.
...
**Proposition:** A separable (i.e. there exists a countable dense subset) Hilbert space has a countable orthonormal basis

This is cool! What this means precisely, is that if I get a seperable Hilbert space $\mathbb{H}$ I can find a basis ${e\_n}\_{n\in I}$ where $I$ is countable, and I can write every element $v\in \mathbb{H}$ like

$$
v = \sum_{n\in I}v_ne_n
$$

where $v\_n = \langle v,e\_n \rangle \in \mathbb{C}$. This not a trivial result at all! We can find a hilbert space made out of functions, and this basically means that we can express each function as a countable sequence of numbers!

We are ready to define our Hilbert space

**Definition:** A **Quantum Hilbert Space** $\mathbb{H}$ is a separable hilbert space with hermitian inner product. (lol)
...
**Definition:** Let $\mathbb{P}(\mathbb{H})$ be be the **projective space** composed of 1 dimensional linear subspaces of $\mathbb{H}$. Namely
...
**Definition:** Given a quantum Hilbert space $\mathbb{H}$ with projective space $\mathbb{P}$ the map

$$
\begin{align*} \delta : \mathbb{P}\times \mathbb{P} &\to [0,1]\\ (f,g) &\mapsto \delta(f,g) = \frac{|\langle f,g\rangle|^2}{|f|^2|g|^2}, \end{align*}
$$
...
**Corollary:** The transition probability defines a topology on $\mathbb{P}$ with basis sets
...
**Defintition:** A bijective map $T:\mathbb{P}\to \mathbb{P}$ is called a **projective transformation** _(or projective automorphism)_ if
...
This is because, while $\text{Aut}(\mathbb{P})$ contains all the _physical_ transformations, we have more degrees of freedom in quantum mechanics. In particular, the Hilbert space is much larger than the projective space, and as we will soon see, all transformations in the projective space originate by transformations in the Quantum Hilbert space. Let’s make this statement more precise now.

**Definition:** Let $q : \mathbb{H}\setminus {0} \to \mathbb{P}$ be the canonical quotient map. Then, for any unitary operator $U \in U(\mathbb{H})$ there exists a canonical representation on the quotient space given by

$$
\hat{U}([f]) = q(Uf) \ \ \forall f \in \mathbb{H}
$$

_Note:_ This is also true for anti unitary operators.

**Theorem:** _(Winger’s Theorem)_ For every projective transformation $T \in \text{Aut}(\mathbb{P})$ there exists a unitary or anti-unitary transformation $U$ such that $T = \hat{U}$.

This statement, although abstract is very powerful. At surface level, it tells us that the following lemma is true

**Lemma:** The group of unitary transformations on the Quantum Hilbert space $\mathbb{H}$ is a central extension of the projective transformations by $U(1)$.
...
…Consider two topological space $(E,T\_E)$ and $(X,T)$. In the following we will study some properties of subsets of $X$ through continious maps from $E$.

**Definition:** Any $U \in T$ is **evenly covered** by a continuous map $q: E \to X$ if $q^{-1}(U)$ is a disjoint union of connected open subsets of $E$. These subsets are called the **sheets** of the covering of $U$, each of which must be homeomorphic to $U$ by a suitable restriction of $q$.
