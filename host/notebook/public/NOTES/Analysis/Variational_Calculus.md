...
There is no special property of functionals that makes them distinct from functions, but using function for both the map and its input could be confusing. Notice that [distributions](../../../../../Analysis/Distributions.md) are functionals, and in fact we will use them a lot in our attempt at calculus.
...
**Example:** Let $M$ be a Lorentzian manifold and consider the Banach space $B \coloneqq C^\infty\_0(M)$ of compactly supported functionas on $M$. Then we can consider real functionals on $B$, ie. $F: B\to \mathbb R$. We know that if $F$ is continuous then it is a distribution (see [distributions](../../../../../Analysis/Distributions.md)). Let $F$ be of the form such that for any $f \in B$
