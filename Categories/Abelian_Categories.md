...
**<u>Theorem:</u>** *(Jordan-Holder Theorem)* If $X$ is an object of finite length, then any filtration of $X$ can be extended to a Jordan Holder series. In addition, any two Jordan-Holder series of $X$ contain any simple object with the same multiplicity, so they have the same length. 
