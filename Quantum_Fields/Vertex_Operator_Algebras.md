...
1.  **Elementwise addition:** For any two $a = (a\_1,a\_2,\cdots), b = (b\_1,b\_2,\cdots) \in R\[\[Z]]$ their sum is the sequence

    $$
    a+b = (a_1+b_1,a_2+b_2, \cdots) \in R[[z]].
    $$
2.  **Scalar multiplication:** For any $a = (a\_1,a\_2,\cdots) \in R\[\[Z]]$ and $c \in \mathbb K$ we have that
...
where $Z\_\ast$ is the set of all words generated by $Z$, with **elementwise addition** and **Cauchy multiplication,** i.e. by Interpreting $a,b \in R\[\[Z]]$ as maps $a,b: \langle Z\rangle \to R$ their product is such that for any $z \in Z\_\ast$

$$
(a\times b)(z) = \sum_{w \in \langle Z\rangle} a(w)b(w^{-1}z).
$$

**Remark:** The ring of formal power series is a ring. Also since $Z\_\ast \subset \langle Z\rangle$ the ring of formal power series is a subset of the vector space of formal distributions.

We often view any element of the formal distribution ring as a sum of terms that belong in the free group. Since we can index $\langle Z\rangle \cong \mathbb Z^{|Z|}$ we can write $a \in R\[\[{z\_1,z\_2,\cdots,z\_n}]]$ as

$$
a = \sum_{j \in \mathbb Z^n} a_j z^j = \sum_{j\in \mathbb Z^n} a_{j_1 j_2\cdots j_n} z_{1}^{j_1}z_{2}^{j_2}\cdots z_{n}^{j_n},
$$

where $a\_{j\_2j\_2\cdots j\_n} \in R$, which looks like a polynomial in the variables $z\_i$. This is a nice interpretation that will help us define the algebraic tools needed in the following sections.
...
where we have used that $n \leq m \iff n\_i \leq m\_i \ \forall i = 1,\cdots, |Z|$.
...
such that $\partial\_z z = 1$, $\partial\_z 1 = 0$ and for any reduced word $w = a\_1a\_2\cdots \in R\[\[Z]]$ we get

$$
\partial_z w = (\partial_z a_1) a_2 \cdots + a_1 (\partial_z a_2)\cdots + \cdots
$$

**Example:** Given a formal distribution $f = \sum\_{k=-\infty}^\infty a\_k z^k$ the derivative is given by
