...
We can alternatively approach the subject constructively by taking a quotient of the tensor algebra $T(V)$ generated by $V$ with the two sided ideal $I(B) := \text{span}\{v\otimes v + Q(v,v) \mid v \in V\}$​. After doing this it takes some tedium to show uniqueness and we have an equivalent definition that I am not gonna write down because it’s long, but I will write this as a corollary. 
...
where $I(B) := \text{span}\{v\otimes v + B(v,v) \mid v \in V\}$ is the two sided ideal defined by the form $B$ and $T(V)$ is the tensor algebra of the vector space $(V,\mathbb{K})$ given by
...
$$
\begin{align*}
T^0(V) &:= \text{span}\{v_1\otimes v_2\otimes \cdots \otimes v_k \mid k \text{ even } v_i \in V \}\\
T^1(V) &:= \text{span}\{v_1\otimes v_2\otimes \cdots \otimes v_k \mid k \text{ odd } v_i \in V \}
\end{align*}
$$
...
$$
\begin{align*}
\text{Cl}^0(V,B) &:= T^0(V)/(T^0(V) \cap I(B))\\
\text{Cl}^1(V,B) &:= T^1(V)/(T^1(V) \cap I(B))
\end{align*}
$$
...
(X,\psi) &\mapsto X\cdot \psi := -i \rho \circ \gamma (X) \psi = X^\mu \gamma_\mu \psi
\end{align*}
$$
...
$$
\text{Cl}^\times(V,B) := \{x \in \text{Cl}(V,B) \mid \exists y \in \text{Cl}(V,B) : xy = yx = 1 \}
$$
...
1. $S_+ ^{s,t} := \{v \in \mathbb{R}^{s,t} \mid \eta(v,v) = + 1\}$
2. $S_- ^{s,t} := \{v \in \mathbb{R}^{s,t} \mid \eta(v,v) = - 1\}$
3. $S_\pm ^{s,t} := S^{s,t}_+ \cup S^{s,t}_-$
...
$$
\text{Pin}(s,t) := \{\gamma(v_1)\gamma(v_2)\cdots \gamma(v_k) \in \text{Cl}(s,t) \mid v_i \in S_\pm, k \in \mathbb{N}\}
$$

The **Spin group** (or Special Pin group) is given by

$$
\text{Spin}(s,t) := \text{Pin}(s,t) \cap \text{Cl}^0(s,t) = \{\gamma(v_1)\gamma(v_2)\cdots \gamma(v_{2k}) \in \text{Cl}(s,t) \mid v_i \in S_\pm, k \in \mathbb{N}\}
$$

Finally, the **Orthochronous Spin Group** is given by

$$
\text{Spin}^+(s,t) := \{\gamma(v_1)\cdots \gamma(v_{2k}) \gamma(w_1)\cdots \gamma(w_{2l}) \in \text{Cl}(s,t)\mid v_i \in S_+, w_i \in S_-, k,l \in \mathbb{N}\}
$$
...
u &\mapsto R_u := R(u,\cdot) \in O(s,t)
\end{align*}
$$
...
$$
\mathfrak{spin}^+(s,t) := \{\gamma(e_i)\gamma(e_j) \in \text{Cl}(s,t) \mid 1 \leq i < j \leq s+t\}
$$
...
$$
\text{Fr}_{GL}(M)_p := \{(v_1,v_2,\cdots, v_n) \subset T_pM \text{ basis}\}
$$

The disjoint union 

$$
\text{Fr}_{GL}(M) := \bigsqcup_{p\in M}\text{Fr}_{GL}(M)_p
$$
...
$$
A_{Sp} := \lambda_\ast^{-1} \circ (\Lambda^\ast A_{SO})
$$
...
\Lambda V = T(V)/I,
$$
where $T(V) := K\oplus V\oplus (V\otimes V)\oplus \cdots$ is the tensor algebra of $V$, and $I \subset T(V)$ is the two-sided ideal given by
$$
