...
**<u>Theorem:</u>** The set of smooth sections $\Gamma^\infty(E)$ of a vector bundle $\pi:E\to M$ with bundle metric $\langle \cdot,\cdot \rangle_E : E\times E \to \mathbb R $ over a compact manifold $M$ is a separable Hilbert space when equipped with the norm $\langle \cdot, \cdot \rangle : \Gamma^\infty(E)\times \Gamma^\infty(E) \to \mathbb R$ given for any two $\phi,\psi \in \Gamma^\infty(E)$ by
$$
...
What we want to do is to define a spinor to be a Grassmann valued field. To do this we will consider the Grassmann algebra generated by $\Gamma^\infty(E)$ which is given by
$$
\Lambda(E) \coloneqq \Lambda \Gamma^\infty(E).
$$
 In fact it has a countable basis generated by any basis $\mathcal B \subset \Gamma^\infty(E)$. 
