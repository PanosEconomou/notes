...
$$
where $W_i$ are the irreducible highest weight representations of $\text{Vir}$ with conformal weight $h$ and central charge $\frac{1}{2}$. However, none of the fields here, other than the stress tensor can be considered to extend the Chiral algebra since the inclusion of any field's modes will make the algebra not closed under OPEs. However we can consider squaring the Ising model! This involves taking the  
